...
#aws ec2 create-image --instance-id $(cat instance.id) --name CNV-Image-Webserver | jq -r .ImageId > image.id
...
#aws ec2 wait image-available --filters Name=name,Values=CNV-Image-Webserver
