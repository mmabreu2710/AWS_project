...
public abstract class ImageProcessingHandler implements HttpHandler, RequestHandler<Map<String,String>, String> {
    ...
    private String handleRequest(String inputEncoded, String format, String requestURI) {
    ...
    public void handle(HttpExchange t) throws IOException {
        ...
        // put parameters into metrics file
        System.out.println("type: " + t.getRequestURI());
        /*
        try {
            PrintWriter writer = new PrintWriter(new FileWriter("metrics.txt", true));
            writer.println("ImageProc parameters");
            writer.println("type: " + t.getRequestURI());
            writer.println("format: " + format);
            writer.println("base64size: " + resultSplits[1].length());
            writer.close();
        } catch (IOException e) {
            System.out.println("An error occurred while writing to the file: " + e.getMessage());
        }*/
        requestedURI = t.getRequestURI().toString();
        String output = handleRequest(resultSplits[1], format, requestedURI);
    ...
    public String handleRequest(Map<String,String> event, Context context) {
        return handleRequest(event.get("body"), event.get("fileFormat"), requestedURI);
    }
}
