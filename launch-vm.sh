...
aws ec2 describe-instances \
    --instance-ids $(cat instance.id) | jq -r ".Reservations[0].Instances[0].PublicIpAddress" > ipv4address
echo "New instance with id $(cat instance.id) has address $(cat instance.dns) and public ip $(cat ipv4address)."
